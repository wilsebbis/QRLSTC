...
Implements incremental trajectory clustering algorithms and utilities for RL-based clustering.
Includes functions for distance calculation, cluster center updates, and cluster dictionary management.
"""
...
def incremental_sp(traj1, traj2, k_dict, k):
    """
    Incremental distance calculation for two trajectories (special case).
    Updates k_dict with mid and real distances, last point, and index.
    Args:
        traj1: list of Point, first trajectory
        traj2: list of Point, second trajectory
        k_dict: dict, stores intermediate results
        k: int, cluster index
    Returns:
        k_dict: updated dictionary
    """
...
def incremental_nsp(traj1, traj2, k_dict, k, i):
    """
    Incremental distance calculation for two trajectories (non-special case).
    Handles various temporal relationships and updates k_dict accordingly.
    Args:
        traj1: list of Point, first trajectory
        traj2: list of Point, second trajectory
        k_dict: dict, stores intermediate results
        k: int, cluster index
        i: int, current index in traj1
    Returns:
        k_dict: updated dictionary
    """
    t1s, t1e, t2s, t2e = traj1[0].t, traj1[-1].t, traj2[0].t, traj2[-1].t
...
def incremental_IED(traj1, traj2, k_dict, k, i, sp_i):
    """
    Wrapper for incremental distance calculation between two trajectories.
    Chooses special or non-special case based on index.
    Args:
        traj1: list of Point, first trajectory
        traj2: list of Point, second trajectory
        k_dict: dict, stores intermediate results
...
def incremental_mindist(trajectory, start_index, current_index, k_dict, cluster_dict, episode):
    """
    Find the minimum incremental distance from a trajectory segment to cluster centers.
    Args:
        trajectory: Traj object
        start_index: int, start index in trajectory
        current_index: int, current index in trajectory
        k_dict: dict, stores intermediate results
        cluster_dict: dict, cluster information
        episode: int, episode index (unused)
    Returns:
        min_dist: float, minimum distance
        cluster_id: int, cluster index with minimum distance
    """
    min_dist = 1e10
    cluster_id = -1
...
def add2clusdict(points, clus_dict, k):
    """
    Add points to cluster dictionary, updating time-based statistics.
    Args:
        points: list of Point
        clus_dict: dict, cluster dictionary
        k: int, cluster index
    """
...
def computecenter(clus_dict, k, threshold_num, threshold_t):
    """
    Compute cluster center points based on time and threshold criteria.
    Args:
        clus_dict: dict, cluster dictionary
        k: int, cluster index
        threshold_num: int, minimum number of points for a time slot
        threshold_t: int, maximum time difference for grouping
    Returns:
        center: list of Point, computed cluster centers
    """
...
def compute_overdist(cluster_dict):
    """
    Compute the average overall distance for all clusters.
    Args:
        cluster_dict: dict, cluster information
    Returns:
        overdist: float, average overall distance
    """
    count = 0
    sumval = 0
    for i in cluster_dict.keys():
...
def update_centers(cluster_dict, threshold_num, threshold_t):
    """
    Update cluster centers for all clusters based on computed centers.
    Args:
        cluster_dict: dict, cluster information
        threshold_num: int, minimum number of points for a time slot
        threshold_t: int, maximum time difference for grouping
    Returns:
        overdist: float, average overall distance
        cluster_dict: dict, updated cluster information
    """
    for i in cluster_dict.keys():
        if len(cluster_dict[i][0])!=0:
