...
Provides functions for calculating distances and similarities between trajectories and segments.
Includes trajectory simplification, model complexity, and time-based slicing utilities.
"""
...
# Small epsilon for numerical stability in segment length calculations
eps = 1e-12   # defined the segment length theta, if length < eps then l_h=0


def makemid(x1, t1, x2, t2, t):
    """
    Linearly interpolate between (x1, t1) and (x2, t2) to get value at time t.
    """
    if (t2 - t1) * (x2 - x1) == 0:
        return x1 + (t - t1)
...
def traj_mdl_comp(points, start_index, curr_index, typed):
    """
    Compute trajectory model complexity or simplification error.
    Args:
        points: list of Point objects
        start_index: int, start point index
        curr_index: int, end point index
        typed: 'simp' for simplification, 'orign' for original
    Returns:
        Model complexity or error (float)
    """
...
def timedTraj(points, ts, te):
    """
    Extracts a sub-trajectory between timestamps ts and te.
    Args:
        points: list of Point objects
        ts: start time
        te: end time
    Returns:
        List of Point objects within the time interval
    """
    if ts == te:
        return
