...
Defines the TrajRLclus class for RL-based trajectory clustering without outlier database.
Handles environment setup, state transitions, reward calculation, and cluster updates.
"""
...
class TrajRLclus():
    """
    RL environment for trajectory clustering (no outlier DB).
    Manages clusters, state transitions, and reward calculation for RL agent.
    """
    def __init__(self, cand_train, base_centers_T, base_centers_E):
        """
        Initialize RL clustering environment.
        Args:
            cand_train: str, path to candidate training trajectories
            base_centers_T: str, path to base cluster centers (training)
            base_centers_E: str, path to base cluster centers (evaluation)
        """
        self.n_actions = 2
        self.n_features = 4
    ...
    def _load(self, cand_train, base_centers_T, base_centers_E):
        """
        Load trajectory and cluster center data from files.
        """
        ...
        # Add cluster center points and dicts
    ...
    def reset(self, episode, label='T'):
        """
        Reset environment for a new episode (trajectory).
        Args:
            episode: int, trajectory index
            label: str, 'T' for training, 'E' for evaluation
        Returns:
        ...
        self.minsim = 0
        self.k = 0
        # Compute initial minimum similarity and cluster assignment
        if label == 'T':
        ...
        # Build initial observation/state vector
        observation = np.array([self.overall_sim, self.minsim, 2 / self.length,
    ...
    def step(self, episode, action, index, label='T'):
        """
        Take an action in the environment (split or continue trajectory).
        Args:
            episode: int, trajectory index
            action: int, 0=continue, 1=split
            index: int, current position in trajectory
            label: str, 'T' for training, 'E' for evaluation
        Returns:
        ...
        """
        if action == 0:
            # Continue trajectory without splitting
            if index + 1 != self.length:
                if label == 'T':
                    ...
                    self.next_minsim, self.k = incremental_mindist(self.trajsdata[episode], self.split_point, index + 1, self.k_dict, self.clusters_E, episode)
                ...
                # Split trajectory at current index
    ...
    def output(self, label='T'):
        """
        Output current clustering results and statistics.
        Args:
            label: str, 'T' for training, 'E' for evaluation
        Returns:
            list: [overall_sim, RW, clusters]
        """
        if label == 'T':
    ...
    def update_cluster(self, label='T'):
        """
        Update cluster centers and reset cluster statistics.
        Args:
            label: str, 'T' for training, 'E' for evaluation
        """
        if label == 'T':
            self.basesim_T, self.clusters_T = update_centers(self.clusters_T, 1, 0.005)
            for i in self.clusters_T.keys():
                self.clusters_T[i][0] = []
                self.clusters_T[i][1] = []
                self.clusters_T[i][3] = defaultdict(list)
        if label == 'E':
            self.basesim_E, self.clusters_E = update_centers(self.clusters_E, 1, 0.005)
